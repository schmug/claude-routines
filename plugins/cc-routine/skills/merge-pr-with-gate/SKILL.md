...
- **Pending / queued / in-progress** → poll `gh pr checks <P> --repo <slug> --watch` (or sleep 60s and re-check) for up to **20 minutes wall time**. If still pending after the budget:
  - Do not apply `needs-you` — pending is not failed.
  - Exit silently. The next `check_suite.completed` (or a manual re-run / re-`pull_request.synchronize`) will re-fire this routine and find CI settled.
...
- Outcome: `auto-merged | escalated | skipped (label) | skipped (CI pending) | skipped (idempotent)`
